## Airflow DAG with Integrated Code

This code provides a complete, self-contained Airflow DAG that integrates all the provided logic. It eliminates the need for separate files by including the Python script and JSON configuration directly within the DAG file.
...
# ------------------------------------------------------------------------------
# In-memory Python script logic
# ------------------------------------------------------------------------------
# You can define a dictionary of global variables to be used by the python_callable.
# This makes the code more organized and easier to pass to the function.
PY_FILE_GLOBALS = {
    "io": io,
    "json": json,
    "logging": logging,
...
def main_task_callable(**context):
    """
    This function contains the entire logic of your original Python script.
    It is designed to be executed by the PythonOperator.
    """
    # Expose the necessary modules and variables to the function's scope
    # This emulates the behavior of a standalone script
    globals().update(PY_FILE_GLOBALS)
    ...
    if not out_gs_uri:
        ...
        ts = ts or datetime.utcnow().strftime("%Y%m%dT%H%M%S")
...
with DAG(
    ...
    run_job = PythonOperator(
        ...
        python_callable=main_task_callable,
